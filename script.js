(() => {
    ...
    const projectDetails = {
        ...
        wireframes: {
            ...
            cip2: 'https://www.figma.com/make/WY0FJ5xT0zeniEFfkXgDBd/CARMELRAY?t=MUGslpzBaUlqZjrK-1',
            portfolio: 'https://www.figma.com/design/Ts5mHaIXjl5yXPZSVRwTxu/My-Portfolio?node-id=0-1&t=yyHqSv8IoeuEnqS6-1'
        }
    };

    const reservedDetailKeys = new Set(['description', 'tech', 'client', 'liveLabel', 'repoLabel']);
    ...
    function collectProjectLinks(details, el) {
        const links = [];
        const seen = new Set();

        const addLink = (label, url) => {
            if (!label || !url || seen.has(label)) return;
            seen.add(label);
        ...
        const liveLabel = el.getAttribute('data-live-label') || details.liveLabel || 'Live';
        const repoLabel = el.getAttribute('data-repo-label') || details.repoLabel || 'Repo';

        addLink(liveLabel, liveUrl);
        addLink(repoLabel, repoUrl);

        Object.entries(details).forEach(([key, value]) => {
            if (!value || typeof value !== 'string') return;
            if (reservedDetailKeys.has(key) || key === 'live' || key === 'repo') return;
            if (/^https?:\/\//.test(value) || /^mailto:/.test(value)) {
                addLink(normalizeLinkLabel(key), value);
            }
        });
    ...
    function openProjectFrom(el) {
        ...
        if (modalLinks) {
            ...
            const linkItems = collectProjectLinks(details, el)
                .map(({ label, url }) => createProjectLink(label, url))
                .filter(Boolean);
            linkItems.forEach((link) => modalLinks.appendChild(link));
